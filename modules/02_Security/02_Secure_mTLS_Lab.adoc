...
.. Select the `Incident Service` workload in the `emergency-response-demo` namespace.
...
// Service mesh mTLS can also be enabled for TCP traffic. The incident service, responder service, and process service use a PostgreSQL database. With OpenShift Service Mesh you can secure the communication between the service and the database.
...
// . Create a `Policy` resource for the PostgreSQL service:
// +
// ----
// $ echo "---
// apiVersion: authentication.istio.io/v1alpha1
...
// . Create a `DestinationRule` resource for the PostgreSQL service:
// +
// ----
// $ echo "---
// apiVersion: networking.istio.io/v1alpha3
...
// . After the PostgreSQL pods redeploy successfully, scale up the incident service and responder service.

// === Configure Process Service PostgreSQL

// . Scale the process service down to `0` pods.
...
// . Create a `Policy` resource for the process service PostgreSQL service:
// +
// ----
// $ echo "---
// apiVersion: authentication.istio.io/v1alpha1
...
// . Create a `DestinationRule` resource for the process service PostgreSQL service:
// +
// ----
// $ echo "---
// apiVersion: networking.istio.io/v1alpha3
...
// . After the PostgreSQL pod redeploys successfully, scale up the process service.
...
// [NOTE]
// ====
// * When using mTLS to the PostgreSQL database, the process service does not always start correctly, and frequently shows database errors in the logs, including the following:
// +
// ----
...
// ----
// +
// The second error requires further investigation.

// * The communication between the mission service and the data grid is not managed by the service mesh. There are several bug reports about Istio mTLS not working with StatefulSets. Also, the communication between the mission service and the data grid uses the HotRod protocol, with support for transport-level security.

// * The communication between the services and Kafka is not managed by the service mesh. Strimzi/Kafka has its own security mechanisms, including transport security and authentication/authorization.
// ====


// == Set Authorization Policy for Services using Istio RBAC

// [WARNING]
// ====
// Since Istio v1.4, _RBAC_ has been deprecated in favor of _AuthorizationPolicy_. However RBAC is still supported for backward compatibility reasons. Hence, the exercises in this lab section will still work, but are no longer the preferred way to define authorization policies for the service mesh.
// The next section of this lab handles authorization policies using AuthorizationPolicy resources.
// Feel free to skip this section, and move directly to the next section.
// ====
...
// Service mesh authorization is enabled using a `ServiceMeshRbacConfig` resource. The authorization policies are defined using a combination of `ServiceRole` and `ServiceRoleBinding` resources. `ServiceRole` defines a list of permissions, which are then bound to one or more `Subjects` using a `ServiceRoleBinding` resource.
...
// . Enable mesh-wide strict mTLS by patching the default `ServiceMeshPolicy` resource to change the mTLS mode from `PERMISSIVE` to `STRICT`:
// +
// ----
...
// .. On the local file system, create a file called `servicemesh-rbac.yml`:
// +
// ----
// $ echo "---
...
// ** The name of the `ServiceMeshRbacConfig` resource must be `default`.
// ** The mode is set to `ON_WITH_INCLUSION`. This means that RBAC is enabled only for the services listed in the `inclusion` whitelist. Other possible values are `ON`, `OFF`, and `ON_WITH_EXCLUSION`.
// ** In this case, service mesh authorization is enabled only for the incident service in the Emergency Response Demo namespace.
// . Create the resource in the service mesh control plane namespace:
// +
// ----
...
// . Create a `ServiceRole` resource for the services in the Emergency Response Demo namespace:
// .. On the local file system, create a file called `sr-incident-service-internal.yml`:
// +
// ----
// $ echo "---
...
// ----
// ** This policy allows calls to the incident service using all HTTP methods.
...
// . Create a `ServiceRoleBinding` resource referencing the `ServiceRole` resource you just created and bind it to all services in the Emergency Response Demo namespace:
// .. On the local file system, create a file called `srb-incident-service-internal.yml`:
// +
// ----
// $ echo "---
// apiVersion: rbac.istio.io/v1alpha1
...
// ----
// ** `curl` requests to the `/incidents` endpoint of the incident service still error out with an HTTP 403 return code.

// . Create a `ServiceRole` resource for external access to the incident service. This service role grants only HTTP GET access to the incident service REST APIs:
// .. On the local file system, create a file called `srb-incident-service-ingress.yml`:
// +
// ----
// $ echo "---
// apiVersion: rbac.istio.io/v1alpha1
...
// ----
// ** This policy allows calls to the incident service using HTTP GET, but disallows other HTTP methods.
// .. Create the `ServiceRole` resource in the Emergency Response Demo namespace:
// +
// ----
...
// . Create a `ServiceRoleBinding` resource referencing the `ServiceRole` resource you just created and bind it to the SPIFFE identity of the Istio ingress gateway service:
// .. On the local file system, create a file called `srb-incident-service-ingress.yml`:
// +
// ----
// $ echo "---
...
// [NOTE]
// ====
// If you have done the previous section of the lab, delete the `ServiceMeshRbacConfig`, `ServiceRole` and `ServiceRoleBinding` resources created.

// ----
// $ oc delete ServiceMeshRbacConfig default -n $SM_CP_NS
// $ oc delete servicerole incident-service-ingress -n $ERDEMO_NS
// $ oc delete servicerole incident-service-internal -n $ERDEMO_NS
// $ oc delete servicerolebinding incident-service-ingress -n $ERDEMO_NS
// $ oc delete servicerolebinding incident-service-internal -n $ERDEMO_NS
// ----
// ====
