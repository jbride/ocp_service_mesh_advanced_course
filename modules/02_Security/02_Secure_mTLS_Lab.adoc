...
IMPORTANT: In real life, the tasks described here will have to be done partially by the controlplane admin, partially by the application owner. Switching between users is tedious, so suggest to perform all tasks as controlplane admin user.
...
$ istioctl -n $RHSM_CONTROL_PLANE_NS authn tls-check ${ISTIO_INGRESSGATEWAY_POD} $ERDEMO_USER-incident-service.$ER_DEMO_NS.svc.cluster.local
----
+
.Sample Output
----
