...
----
+
* Note that `mTLS` is set to `false`. This means that mTLs is not enforced, and services will be able to communicate over plain HTTP.
...
. In the Emergency Response Demo project, verify the istio secrets for key-and-cert have been created:
+
----
...
* Ensure that the health checks are still working when using strict mTLS.
...
** The Service Mesh sidecar injector service can be configured to rewrite HTTP probes at sidecar injection time, so that the requests will be sent to the _Pilot Agent_ in the Envoy sidecar, which will then redirect to the application. This global configuration is set in the ServiceMeshControlPlane CR and will be applied to all pods in the Service Mesh data plane which have the Envoy sidecar injected.
...
. Configure the HTTP based health checks to be rewritten and handled by the pilot agent in the sidecar container. 
...
* Verify that the `rewriteAppHTTPProbe` directive has been added to the `istio-sidecar-injector` ConfigMap:
+
----
...
----
* Once the new pod is up and running, verify the liveness and readiness probes:
+
----
...
** In the sidecar container the _pilot agent_ is listening on port 15020. This is the process that bootstraps the Envoy proxy.
...
* As an alternative, replace the health checks in the DeploymentConfig to use a command based health check with curl rather than a HTTP based health check.
+
----
$ oc edit dc $ERDEMO_USER-incident-service -o yaml -n $ERDEMO_NS
----
...
. Replace the health checks in the Responder Service DeploymentConfig to use a command based health check with curl rather than a HTTP based health check. You can skip this step if you enabled HTTP probe rewriting in the Service Mesh.
+
----
...
. Replace the health checks in the Disaster Simulator DeploymentConfig to use a command based health check with curl rather than a HTTP based health check. You can skip this step if you enabled HTTP probe rewriting in the Service Mesh.
+
----
...
. _VirtualService_ for the Disaster Simulator Service:
+
----
$ echo "---
apiVersion: networking.istio.io/v1alpha3
...
. Replace the health checks in the Incident Priority Service DeploymentConfig to use a command based health check with curl rather than a HTTP based health check. You can skip this step if you enabled HTTP probe rewriting in the Service Mesh.
+
----
...
. Replace the health checks in the Process Service DeploymentConfig to use a command based health check with curl rather than a HTTP based health check. You can skip this step if you enabled HTTP probe rewriting in the Service Mesh.
+
----
...
. Replace the health checks in the Mission Service DeploymentConfig to use a command based health check with curl rather than a HTTP based health check. You can skip this step if you enabled HTTP probe rewriting in the Service Mesh.
+
----
...
. Replace the health checks in the Responder Simulator DeploymentConfig to use a command based health check with curl rather than a HTTP based health check. You can skip this step if you enabled HTTP probe rewriting in the Service Mesh.
+
----
...
. Replace the health checks in the Process Viewer DeploymentConfig to use a command based health check with curl rather than a HTTP based health check. You can skip this step if you enabled HTTP probe rewriting in the Service Mesh.
+
----
...
. Replace the health checks in the Emergency Console DeploymentConfig to use a command based health check with curl rather than a HTTP based health check. You can skip this step if you enabled HTTP probe rewriting in the Service Mesh.
+
----
...
+
----
$ oc patch dc process-service-postgresql --type='json' -p "[{\"op\": \"add\", \"path\": \"/spec/template/metadata\", \"value\": {\"annotations\":{\"sidecar.istio.io/inject\": \"true\"}, \"labels\":{\"app\":\"$ERDEMO_USER-process-service\", \"name\":\"process-service-postgresql\"}}}]" -n $ERDEMO_NS
----
...
With strict mTLS enforced, the Prometheus pod in the `$ERDEMO_USER-er-metrics` namespace cannot longer scrape metric data from the pods in the `$ERDEMO_USER-er-demo` namespace. +
Adding the `$ERDEMO_USER-er-metrics` namespace to the service mesh and injecting the Envoy proxy in the prometheus pod does not work: scraping targets for Prometheus are pod IPs, not service names. The Envoy proxy does not process the traffic properly when directed to a pod IP instead of a service URL. +
Possible solutions: +
...
== Citadel with External CA Certificates and Keys

By default, Citadel uses self-signed certificates for mTLS transport security. Citadel can also be configured to use a certificate and key from an external Certificate Authority (CA). In this lab you will create your own CA, and configure Citadel to use intermediate certificates issued and signed by the external CA.
...
----
.. Create an _openssl_ configuration file:
+
----
$ echo "
[ req ]
...
----
.. Create an _openssl_ configuration file for the intermediate certificate:
+
----
echo "
[ req ]
...
----
. Configure Citadel to use the CA certificates rather than self-signed certificates. This is done by setting the `.spec.istio.security.selfSigned` element in the _ServiceMeshControlPlane_ CR to `false`:
+
----
...
. Delete all the secrets created by Citadel in the Service Mesh data plane. This will force Citadel to recreate the secrets, but this time using the external CA certificates:
+
----
...
. Visualize the contents of the certificate chain in `istio.incident-service` secret. Note that the certificate chain contains three certificates.
+
----
$ oc get secret istio.incident-service -o jsonpath={.data.cert-chain\\.pem} -n $ERDEMO_NS | base64 --decode
----
...
-----END CERTIFICATE-----
----
. Copy the text output of the first certificate. In a browser window, navigate to https://www.sslshopper.com/certificate-decoder.html, and paste the certificate text into the text box:
+
image::images/istio-citadel-ca-certificate.png[]
+
* Note the issuer of the certificate, which corresponds to the intermediate and root CA.

. The application services in the data plane need to be restarted to have the Envoy proxies mount the new secrets.
+
----
...
----
+
If you injected the PostgreSQL pods with the Envoy proxy, make sure to restart these too.

. Test the Emergency Response application to make sure the application is working as expected.