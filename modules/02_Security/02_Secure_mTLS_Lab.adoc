...
.Prerequisites
* Access to the OpenShift 4.x cluster
...
Before starting the lab, you review the Red Hat^(R)^ OpenShift^(R)^ Service Mesh installation.
...
----
+
* Note that `mTLS` is set to `false`. This means that mTLS is not enforced, and services are able to communicate over plain HTTP.
...
+
[NOTE]
The latest versions of upstream Istio also allow HTTP probe rewrites per service by setting an annotation (`sidecar.istio.io/rewriteAppHTTPProbers: "true"`) on the pods. OpenShift Service Mesh does not yet support this functionality.
...
----
* `SERVER` is the mode used on the server. The incident service mTLS policy is set to `PERMISSIVE`, so the status is `HTTP/mTLS`.
...
== Set Authorization Policy for Services
