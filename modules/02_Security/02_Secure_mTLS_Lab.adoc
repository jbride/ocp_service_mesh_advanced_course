...
+
----
$ ISTIO_INGRESSGATEWAY_POD=$(oc get pod -l app=istio-ingressgateway -o jsonpath={.items.metadata.name} -n $SM_CP_NS)
...
spec:
  host: process-service-postgresql.$ERDEMO_NS.svc.cluster.local
  trafficPolicy:
    tls:
      mode: ISTIO_MUTUAL
...
spec:
  ...
  inclusion:
    services:
      - incident-service.$ERDEMO_NS.svc.cluster.local
