...
. Using your favorite text editor, modify the environment variables in this downloaded shell script, as discussed and shared by the instructor
...
While injecting the _Envoy_ service proxy sidecar into an application pod, community Istio typically uses an link:https://kubernetes.io/docs/concepts/workloads/pods/init-containers/[init-container] to manipulate the iptables rules of the node where the pod runs.
...
This normally would not be provided to you in a production environment.
...
Among other features, automatic injection of sidecar containers into a deployment requires an OpenShift _Admission Controller_.
...
+
-----
oc get mutatingwebhookconfiguration --as=system:admin

...

istio-sidecar-injector-admin1-istio-system   2019-11-12T15:12:58Z
istio-sidecar-injector-admin2-istio-system   2019-11-12T15:15:18Z
-----

. Extract the details of your _mutatingwebhookconfiguration_ specific to your Red Hat Service Mesh installation:
+
-----
oc get mutatingwebhookconfiguration istio-sidecar-injector-$RHSM_CONTROL_PLANE_NS -o yaml --as=system:admin > ~/lab/$RHSM_CONTROL_PLANE_NS-mutatingwebhookconfiguration.yaml
-----
+
Study the content of the output file `$RHSM_CONTROL_PLANE_NS-mutatingwebhookconfiguration.yaml` and notice the following:
...
+
-----
oc get statefulset -l strimzi.io/kind=Kafka -n $ER_DEMO_NS
...
+
-----
oc get statefulset -l application=datagrid-service -n $ER_DEMO_NS
...
+
-----
echo -en "\n\n$(oc get route $ERDEMO_USER-emergency-console -o template --template={{.spec.host}} -n $ER_DEMO_NS)\n\n"
-----
