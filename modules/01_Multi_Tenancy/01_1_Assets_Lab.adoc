...
.. link:https://mirror.openshift.com/pub/openshift-v4/clients/oc/4.3/[`oc` utility].

... After installing the `oc` client, make sure it is on your `$PATH` and that its version is in the OpenShift 4.3 family:
+
-----
$ oc version
-----
...
.. link:https://github.com/istio/istio/releases/tag/1.4.6[`istioctl` utility].
** This link provides the same version of the `istioctl` command line utility than the version of Istio that OpenShift Service Mesh 1.1 is based on.
