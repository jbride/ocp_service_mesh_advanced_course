...
+
-----
oc get mutatingwebhookconfiguration istio-sidecar-injector-$SM_CP_NS -o yaml --as=system:admin > ~/lab/$SM_CP_NS-mutatingwebhookconfiguration.yaml
-----
+
Study the content of the output file `$SM_CP_NS-mutatingwebhookconfiguration.yaml` and notice the following:
...
+
-----
echo -en "\n\n$(oc get route $ERDEMO_USER-emergency-console -o template --template={{.spec.host}} -n $ERDEMO_NS)\n\n"
-----
