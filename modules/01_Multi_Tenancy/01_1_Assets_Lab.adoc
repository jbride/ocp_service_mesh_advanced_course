...
== Red Hat Service Mesh global resources
...
While injecting the service proxy sidecar, community Istio typically uses an _init container_ to manipulate the iptables rules of the pod in order to intercept requests to application containers.
Although the service proxy sidecar does not require root to run, the short-lived init container does require _cap_net_admin_ privileges to install iptables rules in each pod just prior to starting the pod's primary containers onboard to the service mesh.
...
Instead, Red Hat Service Mesh makes use of the link:https://istio.io/docs/setup/additional-setup/cni/[istio-cni plugin] for setting up of iptables routing rules to a pod injected with the _Envoy_ side-car container.
The _istio-cni_ container network plugin that adheres to the link:https://github.com/containernetworking/cni[container network interface] specification.

As the CNI pods still run with elevated privileges, they are deployed in the _istio-operator_ namespace (which in your lab environment is only modifiable by a cluster-admin).

To redirect traffic in the application pod’s network namespace to/from the Istio proxy sidecar, the Istio CNI plugin configures the namespace’s iptables.

. View the previously installed _istio-cni_ pods implemented as a OpenShift _DaemonSet_:
+
-----
...
. Notice that Linux container image used is a supported variant of _istio-cni_ :
+
-----
...
. Determine version of community Istio that this version of Red Hat Service Mesh is based on:
+
-----
...
=== Service Mesh _Control Plane_ settings
