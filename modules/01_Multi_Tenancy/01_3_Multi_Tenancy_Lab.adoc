...
* Obtain a remote shell into the _prometheus application_monitoring_ pod in the metrics namespace
+
----
$ oc rsh -c prometheus -n $ERMETRICS_NS prometheus-application-monitoring-0
----
...
. Try to reach the incident service from within another namespace otr from the outside of the cluster using _curl_
+
What do you observe?
...
[NOTE]
====
Execute the following commands you want to restore the networkpolicies to their original state:
