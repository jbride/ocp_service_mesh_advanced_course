...
* Per the diagram above, the `3scale-istio-adapter` Linux container includes the following two components:
...
. In your `incident-service` API, navigate to *Integration -> Configuration* and click *edit APIcast configuration*:
+
...
[NOTE]
====
For this section of the lab to work as expected, you need to delete the `ServiceRole` and `ServiceRoleBinding` resources you created in the Secure mTLS lab in module 2.
...
. In your `incident-service` API, navigate to *Integration -> Configuration* and click *edit APIcast configuration*:
+
image::images/edit_apicast_configs.png[]

. Scroll down to the *MAPPING RULES* section and add a new mapping rule with the following values:
.. *Verb*: `POST`
.. *Pattern*: `/`
+
