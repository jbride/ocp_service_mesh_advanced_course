...
+
----
$ echo -en "\n\nhttps://$(oc get route prometheus -o template --template={{.spec.host}} -n $RHSM_CONTROL_PLANE_NS)\n\n"
----
...
+
----
$ oc get metric -n $RHSM_CONTROL_PLANE_NS
----
+
.Output
----
...
+
----
$ oc get rule requestcount -o yaml -n $RHSM_CONTROL PLANE_NS
----
+
.Output (truncated)
----
...
+
----
$ oc get handler prometheus -o yaml -n $RHSM_CONTROL_PLANE_NS
----
+
.Output (truncated)
----
...
+
----
$ oc get rule -n $RHSM_CONTROL_PLANE_NS
----
+
.Output
----
...
+
----
$ oc get rule promhttp -o yaml -n $RHSM_CONTROL_PLANE_NS
----
+
.Output (truncated)
----
...
+
----
$ echo -en "\n\nhttps://$(oc get route grafana -o template --template={{.spec.host}} -n $RHSM_CONTROL_PLANE_NS)\n\n"
----
...
+
----
$ oc patch dc $ERDEMO_USER-incident-service -p '{"spec":{"template":{"metadata":{"labels":{"version":"v1"}}}}}' -n $ER_DEMO_NS
----
+
...
spec:
  ...
  - imageChangeParams:
      ...
      from:
        ...
        namespace: $ER_DEMO_NS
    type: ImageChange
" | oc create -f - -n $ER_DEMO_NS 
----
+
...
spec:
  ...
  params:
    metrics:
    - name: version_count
      instance_name: version-count.instance.$ER_DEMO_NS
      kind: COUNTER
      label_names:
      - source
      - version
...
spec:
  match: (context.protocol == "http") && (destination.service.host == "user50-incident-service.user50-er-demo.svc.cluster.local")
  actions:
...
+
----
$ oc extract cm/prometheus -n $RHSM_CONTROL_PLANE_NS --to=. --keys=prometheus.yml
----
...
+
----
$ oc delete configmap prometheus -n $RHSM_CONTROL_PLANE_NS
$ oc create configmap prometheus -n $RHSM_CONTROL_PLANE_NS --from-file=prometheus.yml --from-file=ingress.rules --from-file=ingress-alert.rules --save-config=true
$ oc label configmap prometheus -n $RHSM_CONTROL_PLANE_NS app=prometheus app.kubernetes.io/component=prometheus app.kubernetes.io/instance=$RHSM_CONTROL_PLANE_NS app.kubernetes.io/managed-by=maistra-istio-operator app.kubernetes.io/name=prometheus app.kubernetes.io/part-of=istio app.kubernetes.io/version=1.0.1-8.el8-1 chart=prometheus heritage=Tiller maistra.io/owner=$RHSM_CONTROL_PLANE_NS release=istio
----
. Restart the Prometheus pod in the control plane namespace:
+
----
$ oc patch deployment prometheus -p '{"spec":{"template":{"metadata":{"annotations":{"kubectl.kubernetes.io/restartedAt": "'`date -Iseconds`'"}}}}}' -n $RHSM_CONTROL_PLANE_NS
----
