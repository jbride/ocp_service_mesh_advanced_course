...
* A curl request to the `/incidents` endpoint of the Incident Service still returns a HTTP 200 code.
* A curl POST request to the `/incidents` endpoint returns with a HTTP 403 error:
+
----
