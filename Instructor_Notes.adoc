...
.. Wait until all of the machinesets are scaled up and ready.
...
. After running the Ansible, you should see
+
----
...
PLAY RECAP ****************************************************************************************************
localhost                  : ok=46   changed=27   unreachable=0    failed=0    skipped=24   rescued=0    ignored=2
----
...
. Select the route for master. It has `master` in the name
...
.. specify "start_tenant" and "end_tenant" variables. This relates to the number of tenants to create.
...
----
PLAY RECAP ****************************************************************************************************
localhost                  : ok=35   changed=9    unreachable=0    failed=0    skipped=60   rescued=0    ignored=0
----
...
.. Verify that you see two gateways: *stage* and *prod*

. Log into Tenant Account
.. To log into the tenant, move back to 3scale screen
.. Click *Accounts > userX-3scale—xyz*
.. Click the *Admin Domain* 
.. Login with: apiXX / admin. (For example: api1 / admin)
